...
contact.forEach((item) => {
  let sectionItem = arr.find((q) => q.firstletter == item.firstName[0]);
  if (sectionItem) {
    sectionItem.data.push(item.firstName)
  } else {
    let contact1 = {
      firstletter: item.firstName[0],
      data: [item.firstName]
    };
    arr.push(contact1)
  }
    ...
    <View style = {styles.container}>
      <SectionList
        ...
        renderSectionHeader={({ section: { firstletter } }) => (
          <View style={{ paddingHorizontal: 20, backgroundColor: "#D1D0D0" }}>
            <Text style={{ fontSize: 24 }}>{firstletter}</Text>
          </View>
        )}
